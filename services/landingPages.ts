...
export const landingPageService = {
   ...
   */
  async list(organizationId?: string, userId?: string): Promise<LandingPage[]> {
    console.log('📋 [LandingPageService] Listando páginas para org:', organizationId, 'ou user:', userId);
    
    let query = supabase
      .from('landing_pages')
      .select('*');
    
    // Filtrar por organizationId se disponível, senão por userId
    if (organizationId) {
  ...
  async getSavedBlocks(organizationId: string): Promise<SavedBlock[]> {
    const { data, error } = await supabase
      .from('landing_page_blocks')
      .select('*')
      .or(`organization_id.eq.${organizationId},is_template.eq.true`)
