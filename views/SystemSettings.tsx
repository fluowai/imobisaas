...
import { Save, Brain, Check, Info, Activity, Globe } from 'lucide-react';
...
const SystemSettings: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'ai' | 'tracking' | 'domains'>('ai');

  useEffect(() => {
    if (settings?.integrations?.groq?.apiKey) {
      setApiKey(settings.integrations.groq.apiKey);
    }
  }, [settings]);
    ...
    try {
      ...
      await updateSettings({
        ...
        integrations: {
          ...
          groq: {
            apiKey,
            model: 'llama-3.3-70b-versatile' // Default high-perf model
          }
        }
      });
      setSaved(true);
    ...
    <div className="p-6 max-w-4xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500">Gerencie integrações e chaves de API</p>
        </div>
      </div>
