...
import { CONTRACT_TEMPLATES, ContractTemplate } from '../constants/ContractTemplates';
...
const LegalContracts: React.FC = () => {
  ...
  const [contracts, setContracts] = useState<Contract[]>([
    { id: 'cnt-01', title: 'Compra e Venda - Fazenda Solar', type: 'Venda', propertyId: 'f1', clientId: 'l1', propertyName: 'Fazenda Rio Doce', clientName: 'AgroInvest S.A.', clientPhone: '5561988880000', status: 'Active', date: '2024-05-15', value: 12500000, templateId: 'venda-rural' },
    { id: 'cnt-02', title: 'Arrendamento Agrícola', type: 'Arrendamento', propertyId: 'f2', clientId: 'l2', propertyName: 'Sítio Primavera', clientName: 'Carlos Mendes', clientPhone: '5511977776666', status: 'Pending', date: '2024-06-01', value: 45000, templateId: 'arrendamento' },
  ]);
  ...
  const handleCreateContract = (e: React.FormEvent) => {
    e.preventDefault();
    const property = MOCK_PROPERTIES.find(p => p.id === newContract.propertyId);
    const lead = MOCK_LEADS.find(l => l.id === newContract.clientId);
    ...
    const contract: Contract = {
      ...
      type: CONTRACT_TEMPLATES.find(t => t.id === newContract.templateId)?.name || 'Contrato',
      propertyId: newContract.propertyId,
      clientId: newContract.clientId,
      propertyName: property.title,
      clientName: lead.name,
      clientPhone: newContract.clientPhone || lead.phone,
      status: 'Draft',
    ...
    // Se marcou para enviar agora, aguarda a criação e envia
    if (newContract.sendNow) {
      setTimeout(() => sendViaWhatsApp(contract), 500);
    }

    setNewContract({ title: '', propertyId: '', clientId: '', clientPhone: '', templateId: 'venda-rural', value: 0, sendNow: false });
  };

  const sendViaWhatsApp = async (contract: Contract) => {
    if (!settings.integrations?.evolutionApi?.enabled) {
      setWhatsappStatus({ type: 'error', message: 'Evolution API não configurada ou desativada.' });
      return;
    }
    ...
    try {
      ...
      const response = await fetch(`${normalizedBaseUrl}/message/sendText/${instanceName}`, {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        setWhatsappStatus({ type: 'success', message: 'Contrato enviado com sucesso via WhatsApp!' });
      } else {
        const errorData = await response.json().catch(() => ({}));
        setWhatsappStatus({ type: 'error', message: `Erro na API: ${errorData.message || response.statusText || 'Não autorizado/Erro no servidor'}` });
      }
    } catch (error) {
       console.error("Erro ao enviar WhatsApp:", error);
       setWhatsappStatus({ type: 'error', message: 'Falha de conexão: Verifique se a URL da API está correta e se a instância está conectada.' });
    } finally {
  ...
  const getGeneratedContent = (contract: Contract) => {
    ...
    return template.content
      ...
      .replace(/{{property_area}}/g, property ? (property.features.area / 10000).toString() : '0')
    ...
    <div className="space-y-10 pb-20 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          <h1 className="text-4xl font-black text-black uppercase italic tracking-tighter leading-none mb-3">
             Gestão <span style={{ color: settings.primaryColor }}>Jurídica</span>
          </h1>
          <p className="text-black/60 font-medium italic">Monitoramento de contratos, escrituras e conformidade legal.</p>
        </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-slate-50">
                <th className="px-10 py-6 text-left text-[10px] font-black uppercase tracking-widest text-black/30">Documento</th>
                <th className="px-6 py-6 text-left text-[10px] font-black uppercase tracking-widest text-black/30">Propriedade</th>
                <th className="px-6 py-6 text-left text-[10px] font-black uppercase tracking-widest text-black/30">Cliente</th>
                <th className="px-6 py-6 text-left text-[10px] font-black uppercase tracking-widest text-black/30">Status</th>
                <th className="px-10 py-6 text-right text-[10px] font-black uppercase tracking-widest text-black/30">Ações</th>
              </tr>
            </thead>
                ...
                <tr key={contract.id} className="hover:bg-slate-50 transition-colors group">
                  <td className="px-10 py-6">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="font-black text-black text-sm">{contract.title}</p>
                        <p className="text-[10px] font-bold text-black/30 uppercase tracking-widest">{contract.type}</p>
                      </div>
                    </div>
                  </td>
                  ...
                  <td className="px-6 py-6">
                    <span className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest border ${getStatusStyle(contract.status)} shadow-sm`}>
                  ...
                  <td className="px-10 py-6 text-right">
                    <div className="flex items-center justify-end gap-2">
                       <button 
                        onClick={() => { setSelectedContract(contract); setIsGeneratorOpen(true); }}
      ...
      {isCreateModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
          <div className="absolute inset-0 bg-black/60 backdrop-blur-md" onClick={() => setIsCreateModalOpen(false)}></div>
          <div className="relative bg-white w-full max-w-2xl rounded-[3rem] shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-300">
             <div className="p-12">
                <div className="flex justify-between items-center mb-10">
                   <h2 className="text-2xl font-black text-black uppercase italic tracking-tighter">Novo <span style={{ color: settings.primaryColor }}>Contrato Rural</span></h2>
                ...
                <form onSubmit={handleCreateContract} className="space-y-6">
                   <div className="space-y-2">
                      <label className="text-[10px] font-black uppercase text-black/40 tracking-[0.2em] ml-4">Título do Documento</label>
                      <input 
                        required
                        type="text" 
                   ...
                   <div className="grid grid-cols-2 gap-6">
                      <div className="space-y-2">
                        <label className="text-[10px] font-black uppercase text-black/40 tracking-[0.2em] ml-4">Propriedade</label>
                        <select 
                          required
                        ...
                        >
                          ...
                          {MOCK_PROPERTIES.map(p => <option key={p.id} value={p.id}>{p.title}</option>)}
                        </select>
                      </div>
                      <div className="space-y-2">
                        <label className="text-[10px] font-black uppercase text-black/40 tracking-[0.2em] ml-4">Cliente / Lead</label>
                        <select 
                          required
                          className="w-full px-8 py-4 bg-slate-50 rounded-2xl border-none outline-none font-bold text-sm appearance-none cursor-pointer"
                          value={newContract.clientId}
                          onChange={(e) => {
                            const lead = MOCK_LEADS.find(l => l.id === e.target.value);
                            setNewContract({...newContract, clientId: e.target.value, clientPhone: lead?.phone || ''});
                          }}
                        >
                          <option value="">Selecionar...</option>
                          {MOCK_LEADS.map(l => <option key={l.id} value={l.id}>{l.name}</option>)}
                        </select>
                      </div>
                   </div>

                   <div className="space-y-2">
                      <label className="text-[10px] font-black uppercase text-black/40 tracking-[0.2em] ml-4">WhatsApp do Cliente (Com DDD)</label>
                      <input 
                        required
                        type="tel" 
                   ...
                   <div className="grid grid-cols-2 gap-6">
                      <div className="space-y-2">
                        <label className="text-[10px] font-black uppercase text-black/40 tracking-[0.2em] ml-4">Template</label>
                        <select 
                          required
                          className="w-full px-8 py-4 bg-slate-50 rounded-2xl border-none outline-none font-bold text-sm appearance-none cursor-pointer"
                      ...
                      <div className="space-y-2">
                        <label className="text-[10px] font-black uppercase text-black/40 tracking-[0.2em] ml-4">Valor (R$)</label>
                        <input 
                          type="number"
                          required
                   ...
                   <div className="flex items-center gap-3 px-4 py-3 bg-emerald-50 rounded-2xl border border-emerald-100">
                      <input 
                        ...
                        onChange={(e) => setNewContract({...newContract, sendNow: e.target.checked})}
                      />
                      <label htmlFor="sendNow" className="text-xs font-black uppercase text-emerald-800 tracking-widest cursor-pointer select-none">
      ...
      {isGeneratorOpen && selectedContract && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-10">
          <div className="absolute inset-0 bg-slate-900/40 backdrop-blur-xl" onClick={() => setIsGeneratorOpen(false)}></div>
          <div className="relative bg-white w-full max-w-5xl h-full max-h-[90vh] rounded-[3rem] shadow-2xl flex flex-col overflow-hidden animate-in slide-in-from-bottom-10 duration-500">
             ...
             <div className="p-8 border-b border-slate-100 flex justify-between items-center shrink-0">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="text-xl font-black text-black uppercase italic tracking-tighter leading-none">Minuta Inteligente</h3>
                    <p className="text-[10px] font-bold text-black/40 uppercase tracking-widest mt-1">Ref: {selectedContract.id}</p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                   ...
                   <button onClick={() => setIsGeneratorOpen(false)} className="p-3 hover:bg-slate-100 rounded-full text-black/20 hover:text-black transition-all ml-4">
                      <X size={20} />
                   </button>
                </div>
             </div>
             ...
             {whatsappStatus && (
               <div className={`mx-8 mt-4 p-4 rounded-2xl flex items-center justify-between animate-in slide-in-from-top-4 duration-300 ${whatsappStatus.type === 'success' ? 'bg-emerald-50 text-emerald-700 border border-emerald-100' : 'bg-red-50 text-red-700 border border-red-100'}`}>
                 <div className="flex items-center gap-3">
                   {whatsappStatus.type === 'success' ? <CheckCircle2 size={18} /> : <AlertCircle size={18} />}
                   <span className="text-xs font-bold">{whatsappStatus.message}</span>
                 </div>
                 <button onClick={() => setWhatsappStatus(null)} className="p-1 hover:bg-black/5 rounded-full transition-colors">
                   <X size={14} />
                 </button>
               </div>
             )}
                ...
                <div className="max-w-3xl mx-auto bg-white p-20 shadow-xl border border-slate-100 rounded-[1rem] min-h-full font-serif text-slate-800 leading-relaxed text-lg">
                   ...
                   <div className="mt-32 pt-20 border-t border-slate-100 flex justify-between gap-10">
                      <div className="flex-1 border-b border-black/20 pb-2 text-center">
                         <p className="text-xs font-black uppercase tracking-[0.3em] mb-1">Assinatura Vendedor</p>
                         <p className="text-[10px] text-black/30">Fazendas Brasil Select</p>
                      </div>
                      <div className="flex-1 border-b border-black/20 pb-2 text-center">
                         <p className="text-xs font-black uppercase tracking-[0.3em] mb-1">Assinatura Comprador</p>
                         <p className="text-[10px] text-black/30">{selectedContract.clientName}</p>
                      </div>
                   </div>
                </div>
             </div>
